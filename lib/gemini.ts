import { callGeminiApi } from "@/app/actions/gemini-actions" // Import the server action
import type { Task } from "@/types" // Assuming Task type is defined in @/types

// This function now calls the server action. It can remain client-side callable.
...
`
  ...
  try {
    // This now calls the local generateAIResponse, which in turn calls the server action
